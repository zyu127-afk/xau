...
def main():
    ...
    for i in range(60): mapping.add(2600+i*0.1, 2590+i*0.1002)
    estimate=mapping.estimate()
    assert estimate is not None
    bars=[]; p=2500.0
    for i in range(80):
        d=2 if (i//8)%2==0 else -0.7
        o=p; c=p+d; bars.append(Bar(str(i),o,max(o,c)+1,min(o,c)-1,c,100+i)); p=c
    state=classify_market({tf:bars for tf in ('D1','H4','H1','M30','M15','M5','M1')})
    flow=OrderFlowEngine()
    for _ in range(20): flow.ingest({'type':'trade','payload':{'volume':5,'aggressor_side':'BUY'}})
