    ...
    def assess(self, lookback: int = 200) -> OrderFlowAssessment:
        ...
        for event in recent:
            ...
            if t in {"dom", "depth", "book"}:
                dom_bid += _number(p.get("bid_size"))
                dom_ask += _number(p.get("ask_size"))
            ...
            if t in {"sweep", "order_flow_signal"}:
                ...
                if "up" in detail or side == "BUY":
                    score += 0.6 * strength; evidence.append("向上主动扫单")
                elif "down" in detail or side == "SELL":
                    score -= 0.6 * strength; evidence.append("向下主动扫单")
            if t in {"exhaustion", "order_flow_signal"}:
                detail = str(p.get("event_type", p.get("detail", ""))).lower()
                if "seller" in detail or "sell" in detail:
                    score += 0.8 * strength; evidence.append("卖方衰竭")
                elif "buyer" in detail or "buy" in detail:
                    score -= 0.8 * strength; evidence.append("买方衰竭")
            if t in {"liquidity_stack", "liquidity_pull", "order_flow_signal"}:
                detail = str(p.get("event_type", p.get("detail", ""))).lower()
                if "bid_stack" in detail or "ask_pull" in detail:
                    score += 0.45 * strength; evidence.append("买侧流动性增强")
                elif "ask_stack" in detail or "bid_pull" in detail:
                    score -= 0.45 * strength; evidence.append("卖侧流动性增强")
            if t in {"imbalance", "footprint", "order_flow_signal"}:
                imbalance = _number(p.get("imbalance", p.get("delta_ratio", 0.0)))
                if imbalance > 0.15:
                    score += min(1.0, imbalance) * 0.7; evidence.append("Footprint买方不平衡")
                elif imbalance < -0.15:
                    score += max(-1.0, imbalance) * 0.7; evidence.append("Footprint卖方不平衡")
            if t in {"iceberg", "order_flow_signal"}:
                detail = str(p.get("event_type", p.get("detail", ""))).lower()
                if "bid" in detail or "buy" in detail:
                    score += 0.5 * strength; evidence.append("买侧疑似冰山")
                elif "ask" in detail or "sell" in detail:
                    score -= 0.5 * strength; evidence.append("卖侧疑似冰山")
