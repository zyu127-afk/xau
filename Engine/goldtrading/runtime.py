...
from .event_outcomes import FastEventOutcomeTracker
...
class Runtime:
    def __init__(self, settings: Settings, db: Database) -> None:
        ...
        self.event_outcomes = FastEventOutcomeTracker()
    ...
    async def _on_atas(self, event: dict) -> None:
        ...
        event_uid = self.event_outcomes.add(event, mapped)
        stored = dict(event)
        if event_uid:
            stored["event_uid"] = event_uid
        self.db.execute("INSERT INTO OrderFlowEvents(ts,event_type,gc_price,mt5_price,strength,payload) VALUES(?,?,?,?,?,?)",
                        (now, str(event.get("type", "unknown")), gc_price, mapped, strength, json.dumps(stored, ensure_ascii=False)))
    ...
    def _zone_text(items: object) -> str:
        ...
        try: return f"{float(zone['low']):.2f}-{float(zone['high']):.2f}"
    ...
    def _potential_text(items: object, side: str) -> str:
        ...
        for item in items:
            if not isinstance(item, dict) or str(item.get("side", "")).upper() != side: continue
            zone = item.get("zone") if isinstance(item.get("zone"), dict) else {}
            try: return f"{float(zone['low']):.2f}-{float(zone['high']):.2f} {item.get('status','')}"
            except (KeyError, TypeError, ValueError): return "-"
        return "-"
    ...
    async def _publish_mt5_hud(self, degradation_name: str) -> None:
        ...
        fields = [
            d.get("regime", "UNKNOWN"), d.get("bias", "NEUTRAL"),
            self._zone_text(d.get("supports")), self._zone_text(d.get("resistances")),
            self._potential_text(d.get("zones"), "BUY"), self._potential_text(d.get("zones"), "SELL"),
            d.get("orderflow_assessment", "数据不足"), system.get("ai", self.ai_status),
            "-" if self.ai_latency_ms is None else f"{self.ai_latency_ms:.0f}ms", degradation_name,
        ]
        await self.guardian.publish_status(fields)

    def _persist_event_outcomes(self, current_mid: float) -> None:
        for outcome in self.event_outcomes.evaluate(current_mid):
            payload = asdict(outcome)
            self.db.execute(
                "INSERT OR REPLACE INTO OrderFlowOutcomes(event_uid,horizon_seconds,ts,event_type,instrument,gc_price,start_mt5,end_mt5,move,strength,payload) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                (outcome.event_uid, outcome.horizon_seconds, outcome.ts_utc, outcome.event_type, outcome.instrument,
                 outcome.gc_price, outcome.start_mt5, outcome.end_mt5, outcome.move, outcome.strength,
    ...
    async def status_loop(self) -> None:
        while True:
            ...
            current_mid = None
            if self.guardian.state.bid is not None and self.guardian.state.ask is not None:
                current_mid = (self.guardian.state.bid + self.guardian.state.ask) / 2.0
                self._persist_event_outcomes(current_mid)
            if raw_gc is not None and current_mid is not None and atas_fresh:
                self.alignment.add(float(raw_gc), current_mid)
            ...
            await self.dashboard.push({
                ...
                "market": {
                    ...
                    "bid": self.guardian.state.bid, "ask": self.guardian.state.ask,
                    "spread": self.guardian.state.spread_points,
                    "atas_contract": self.atas.state.instrument, "gc_price": raw_gc,
                ...
                "recent_fast_event_outcomes": self.event_outcomes.recent(20),
            ...
            if self.guardian.heartbeat_fresh() and now_mono - self._last_slot_persist >= 10.0:
                persist_guardian_slots(self.db, self.guardian.state); self._last_slot_persist = now_mono
        ...
        while True:
            try:
                if self.guardian.heartbeat_fresh() and self.guardian.state.bid is not None and self.guardian.state.ask is not None:
                    now = time.monotonic(); assessment = self.orderflow.assess()
                    for name in ("A", "B"):
                        ...
                        if decision.action == "HOLD" and slot.mfe > 0:
                            if slot.side.upper() == "BUY" and assessment.score <= -1.5:
                                ...
                                decision.action, decision.reason = "CLOSE", "strong opposing order flow"
                        if decision.action == "HOLD" or now - self._last_management_action[name] < 2.0: continue
    ...
    async def retention_loop(self) -> None:
        while True:
            ...
            if deleted: self.log.info("90-day rolling retention removed %d raw/detail rows", deleted)
            await asyncio.sleep(6 * 3600)

    async def _connect_mt5_data(self) -> None:
        if self.mt5_data is None:
            self.log.warning("MT5 Python data adapter disabled"); return
    ...
    async def run(self) -> None:
        checker = StartupChecker(self.settings); checks = checker.run_local_checks()
        for check in checks:
        ...
        if not checker.critical_ok(checks): raise RuntimeError("critical local startup checks failed")
        await self.guardian.start(); await self._connect_mt5_data()
        workers = [
        ...
        try:
            ...
            await self.atas.stop(); await self.guardian.close()
            if self.mt5_data is not None: await asyncio.to_thread(self.mt5_data.shutdown)
