...
class DynamicPositionManager:
    """Deterministic management layer. It may tighten protection or exit; it never widens risk."""
    ...
    def __init__(self, break_even_r: float = 1.0, lock_r: float = 1.5, lock_fraction_r: float = 0.5,
        ...
        self.lock_r = lock_r
        self.lock_fraction_r = lock_fraction_r
        self.giveback_trigger_r = giveback_trigger_r
        self.max_giveback_fraction = max_giveback_fraction

    def evaluate(self, slot: SlotTelemetry, market_price: float) -> ManagementDecision:
        if not slot.active or slot.entry_price <= 0 or slot.original_sl <= 0 or slot.current_sl <= 0:
            return ManagementDecision("HOLD")
        side = slot.side.upper()
        if side not in {"BUY", "SELL"}:
            return ManagementDecision("HOLD")
        initial_r = abs(slot.entry_price - slot.original_sl)
        if initial_r <= 0:
            return ManagementDecision("HOLD")
        favorable = market_price - slot.entry_price if side == "BUY" else slot.entry_price - market_price
        r_now = favorable / initial_r
        mfe_r = slot.mfe / initial_r
        if mfe_r >= self.giveback_trigger_r and slot.mfe > 0:
            giveback = max(0.0, slot.mfe - favorable) / slot.mfe
            if giveback >= self.max_giveback_fraction:
                return ManagementDecision("CLOSE", reason=f"MFE giveback {giveback:.0%}")
        ...
        if abs(candidate - slot.current_sl) > 1e-9:
            return ManagementDecision("MODIFY_SL", candidate, "profit protection")
        return ManagementDecision("HOLD")
    ...
    def command(slot_name: str, slot: SlotTelemetry, decision: ManagementDecision, price: float) -> GuardianCommand:
        ...
        return GuardianCommand(
            command_id=f"pm-{slot_name}-{int(now.timestamp()*1000)}",
            action="CLOSE" if decision.action == "CLOSE" else "MODIFY_SL",
            slot=slot_name,
            side=slot.side,
            lot=slot.lot,
            stop_loss=decision.stop_loss or slot.current_sl,
            take_profit=slot.current_tp or None,
            zone_low=price - 1e9,
            zone_high=price + 1e9,
